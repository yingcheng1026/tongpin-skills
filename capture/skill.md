---
name: capture
description: 知识捕获与学习系统。用户给链接时触发，智能判断意图，自动选择「快速存档」或「深度学习」模式。触发词：保存/收藏/存档/记录/捕获/学习/理解 + 链接，或任何包含保存类动词的请求。
...
### ⚡️ 自动触发（重要！）
**任何时候用户给出链接并表达保存意图，都必须触发此 skill！**

用户说以下任何内容时自动触发：
- `/(保存|收藏|存档|记录|存一下|收藏一下|保存一下|存一下|记录一下).*/`
- `/(捕获|学习|理解|消化).*/`
- `/(帮我处理|帮我保存|帮我存).*/`
- 直接粘贴 URL（如果没有其他明确指令）
